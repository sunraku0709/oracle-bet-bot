...
def oracle_analyse(home, away, sport, market, selection, odds_by_book):
    ...
    prompt = (
        f"Tu es ORACLE, analyste de paris sportifs expert.\n\n"
        f"MATCH : {home} vs {away}\n"
        f"SPORT : {sport}\n"
        f"MARCHE : {market}\n"
        f"SELECTION : {selection}\n"
        f"COTES REELLES : {odds_text}\n"
        f"MEILLEURE COTE : {best_odds:.2f}\n\n"
        f"ANALYSE STRUCTUREE :\n\n"
        f"1. FORME RECENTE (5 derniers matchs)\n"
        f"- Resultats V/N/D avec scores\n"
        f"- Buts/points marques et encaisses\n"
        f"- Dynamique : progression / chute / stagnation\n\n"
        f"2. H2H (5 derniers)\n"
        f"- Tendances reelles uniquement\n"
        f"- Si aucune tendance : Aucune tendance H2H exploitable\n\n"
        f"3. MATCH-UP CLE\n"
        f"- Points forts / faibles\n"
        f"- Avantage tactique pour ce marche\n\n"
        f"4. ABSENCES ET IMPACT\n"
        f"- Joueurs cles absents\n"
        f"- Si inconnu : Aucune source fiable disponible\n\n"
        f"5. ENJEUX\n"
        f"- Classement et objectif du match\n\n"
        f"6. RED FLAGS\n"
        f"- Fatigue / rotation / match sans enjeu\n\n"
        f"7. SYNTHESE\n"
        f"- 3 a 5 facteurs decisifs uniquement\n\n"
        f"8. PRONOSTIC FINAL\n"
        f"- Probabilite estimee en pourcentage\n"
        f"- Niveau de risque : Faible / Moyen / Eleve\n"
        f"- VALUE BET : oui ou non\n"
        f"- GOLD (75 pct+) / SILVER (65-74 pct) / NO BET\n\n"
        f"REGLES : Zero blabla. Zero supposition. Donnees uniquement."
    )
    try:
