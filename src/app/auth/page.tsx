...
function AuthContent() {
  ...
  const redirectToCheckout = async (planId: PlanId) => {
    try {
      ...
      setMessage({ type: 'error', text: data.error || 'Erreur paiement Stripe. Réessayez depuis la page abonnement.' })
      setLoading(false)
    } catch {
      setMessage({ type: 'error', text: 'Erreur réseau lors du paiement. Réessayez depuis la page abonnement.' })
      setLoading(false)
    }
  }
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!email) { setMessage({ type: 'error', text: 'Veuillez saisir votre adresse email.' }); return }
    setLoading(true)
    setMessage(null)

    const redirectTo =
      (typeof window !== 'undefined' ? window.location.origin : 'https://oracle-bet-bot.vercel.app')
    ...
    if (error) {
      ...
      setMessage({
        type: 'success',
        text: 'Email envoyé ! Vérifiez votre boîte mail et cliquez sur le lien de réinitialisation.',
      })
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'register') {
        ...
        const data = await res.json()
        if (!res.ok) { setMessage({ type: 'error', text: data.error || "Erreur lors de l'inscription." }); setLoading(false); return }
        const { error: signInError } = await supabase.auth.signInWithPassword({ email, password })
        if (signInError) { setMessage({ type: 'success', text: 'Compte créé ! Connectez-vous maintenant.' }); setMode('login'); setLoading(false); return }
        ...
        if (planParam && signInData.user) {
          const { data: existingSub } = await supabase.from('subscriptions').select('status')
            .eq('user_id', signInData.user.id).eq('status', 'active').single()
          if (existingSub) { router.push('/dashboard') } else { await redirectToCheckout(getPlanById(planParam)) }
        } else {
          router.push('/dashboard')
        }
      }
    } catch (err) {
      setMessage({ type: 'error', text: 'Erreur réseau : ' + (err instanceof Error ? err.message : 'Erreur inconnue') })
      setLoading(false)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] flex flex-col items-center justify-center px-4">
      ...
      <div className="fixed inset-0 opacity-[0.03] pointer-events-none"
        style={{ backgroundImage: 'linear-gradient(#C9A84C 1px, transparent 1px), linear-gradient(90deg, #C9A84C 1px, transparent 1px)', backgroundSize: '50px 50px' }} />
      ...
      <div className="glass-card rounded-2xl p-8 w-full max-w-md relative z-10">
        ...
        {mode === 'forgot' ? (
          <>
            <button onClick={() => { setMode('login'); setMessage(null) }}
              className="flex items-center gap-1.5 text-white/40 hover:text-[#C9A84C] text-xs mb-6 transition-colors">
            ...
            <form onSubmit={handleForgotPassword} className="space-y-4">
              ...
              {message && (
                <div className={`p-3 rounded-lg text-sm ${message.type === 'error' ? 'bg-red-500/10 border border-red-500/30 text-red-400' : 'bg-green-500/10 border border-green-500/30 text-green-400'}`}>
                  {message.text}
                </div>
              )}

              <button type="submit" disabled={loading}
                className="btn-gold w-full py-3.5 rounded-xl text-sm tracking-widest disabled:opacity-50 disabled:cursor-not-allowed">
          ...
          <>
            ...
            <div className="flex mb-8 bg-white/5 rounded-xl p-1">
              {(['login', 'register'] as const).map((m) => (
                <button key={m} onClick={() => { setMode(m); setMessage(null) }}
                  className={`flex-1 py-2.5 rounded-lg font-semibold text-sm tracking-widest transition-all ${mode === m ? 'btn-gold' : 'text-gray-400 hover:text-white'}`}>
            ...
            {mode === 'register' && <PlanPicker selected={selectedPlan} onSelect={setSelectedPlan} />}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-xs font-semibold text-gray-300 tracking-wider uppercase">
                    Mot de passe
                  </label>
                  {mode === 'login' && (
        ...
        <div className="mt-6 pt-6 border-t border-white/5 text-center">
          <p className="text-xs text-gray-600">⚠️ Les paris sportifs comportent des risques. +18 uniquement.</p>
        </div>
      </div>
