...
const faqs = [
  ...
  {
    question: "Combien d'analyses puis-je faire par jour ?",
    answer:
      "Cela dépend de votre plan : Starter = 1 analyse/jour, Standard = 4 analyses/jour, Premium = illimité. Vous pouvez changer de plan à tout moment.",
  },
  {
    question: "Dois-je payer pour créer un compte ?",
    answer:
      "Non. Vous créez votre compte gratuitement, choisissez votre plan, puis effectuez le paiement via Stripe à l'étape suivante. Aucune carte requise à l'inscription.",
  },
  {
  ...
  },
  {
    ...
    answer:
      "Oui, l'IA a accès aux données les plus récentes disponibles (compositions, blessures, forme, cotes) pour chaque analyse.",
  },
  {
            ...
            return (
              <div
                ...
                className={`glass-card rounded-xl transition-all duration-300 ${
                  isOpen ? "border-[#C9A84C]/40" : "hover:border-[#C9A84C]/25"
                }`}
              >
                <button
                  onClick={() => toggle(i)}
                ...
                >
                  <span
                    className={`text-sm font-medium tracking-wide transition-colors duration-200 ${
                      isOpen ? "text-[#C9A84C]" : "text-white/75 group-hover:text-white"
                    }`}
                  >
                    {faq.question}
                  </span>
                  <span
                    className={`shrink-0 w-6 h-6 flex items-center justify-center rounded-full border transition-all duration-300 ${
                      isOpen
                        ...
                        : "border-white/15 group-hover:border-[#C9A84C]/40"
                    }`}
                  >
                    <svg width="10" height="10" viewBox="0 0 10 10" fill="none"
                      className={isOpen ? "text-[#C9A84C]" : "text-white/40"}>
                ...
                <div className={`overflow-hidden transition-all duration-300 ${isOpen ? "max-h-60 opacity-100" : "max-h-0 opacity-0"}`}>
                  <p className="px-6 pb-5 text-sm text-white/50 leading-relaxed border-t border-white/[0.06] pt-4">
                    {faq.answer}
                  </p>
                </div>
              </div>
